...
def product_category_create(request):
    if request.method == 'POST':
        ...
        if form_data.is_valid():
            form_data.save()
            return redirect('category.create') #same page redirection after form submission
...
def product_create(request):
    """
    This view is foe creating a new product in the database. It handles both POST and GET requests.
...
def product_index(request):
    """
    This view is respomsible for displaying all the products in the database
    """
...
def product_edit(request, pk):
    ...
    GET: Renders the product edit form with the existing product data.
    POST: Validates and saves the updated product data to the database.
    """
    ...
    if request.method == 'POST':
        ...
        if form_data.is_valid():
            form_data.save()
            return redirect('product.index')  # Redirect to the product index page after successful edit
    else:
        #re-using product_form.html template for product edit
...
def product_delete(request, pk):
    ...
    GET: Renders a confirmation page for product deletion.
    POST: Deletes the product from the database and redirects to the product index page.
    """
    product = Product.objects.get(pk=pk)
    
    if request.method == 'POST':
        product.delete()
        return redirect('product.index')  # Redirect to the product index page after successful deletion
    else:
        context = {
            "product": product,
